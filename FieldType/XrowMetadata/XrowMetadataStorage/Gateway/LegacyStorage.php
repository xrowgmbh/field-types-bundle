...
namespace xrow\FieldTypesBundle\FieldType\XrowMetadata\XrowMetadataStorage\Gateway;
...
class LegacyStorage extends Gateway
{
    ...
    {
        $this->assignContentAttribute($field);
    ...
    public function assignContentAttribute(Field $metadata)
    {
        $xmlString = $this->saveXML( $metadata->value->externalData );
        
        $connection = $this->getConnection();
        
        $updateQuery = $connection->createUpdateQuery();
        $updateQuery->update( $connection->quoteTable( 'ezcontentobject_attribute' ) )
        ->set(
            $connection->quoteColumn( 'data_text' ),
            $updateQuery->bindValue( $xmlString )
        )->where(
            $updateQuery->expr->lAnd(
                $updateQuery->expr->eq(
                    $connection->quoteColumn( 'id' ),
                    $updateQuery->bindValue( $metadata->id, null, \PDO::PARAM_INT )
                ),
                $updateQuery->expr->eq(
                    $connection->quoteColumn( 'version' ),
                    $updateQuery->bindValue( $metadata->versionNo, null, \PDO::PARAM_INT )
                )
            )
        );
    ...
    public function saveXML( $keywords )
    {
        $xml = new DOMDocument( "1.0", "UTF-8" );
        $xmldom = $xml->createElement( "MetaData" );
        $node = $xml->createElement( "title", htmlspecialchars( "", ENT_QUOTES, 'UTF-8' ) );
        $xmldom->appendChild( $node );
        $node = $xml->createElement( "keywords", htmlspecialchars( implode(',', $keywords) , ENT_QUOTES, 'UTF-8' ) );
        $xmldom->appendChild( $node );
        $node = $xml->createElement( "description", htmlspecialchars( "", ENT_QUOTES, 'UTF-8' ) );
        $xmldom->appendChild( $node );

        $node = $xml->createElement( "priority" );
        $xmldom->appendChild( $node );
        $node = $xml->createElement( "change", htmlspecialchars( "daily", ENT_QUOTES, 'UTF-8' ) );
        $xmldom->appendChild( $node );
        $node = $xml->createElement( "sitemap_use", htmlspecialchars( "1", ENT_QUOTES, 'UTF-8' ) );
        $xmldom->appendChild( $node );
